    ...
    const Node& texture)
{
    ...
    if (dataNode->dataSize < 0x20)
        throw std::runtime_error(
            "texdat record too small: " + texture.name);
    ...
    if (info.width == 0 || info.height == 0)
        throw std::runtime_error(
            "invalid zero-sized texture: " + texture.name);
    ...
    if (pixelCount > 0xFFFFFFFFull)
        throw std::runtime_error(
            "texture dimensions overflow: " + texture.name);

    if (static_cast<std::uint64_t>(info.texelBytes) != pixelCount)
        throw std::runtime_error(
            "texture is not the expected 8-bit indexed layout: " +
            texture.name);

    const std::uint64_t expected =
        0x20ull +
        static_cast<std::uint64_t>(info.texelBytes) +
        0x80ull;

    if (expected != dataNode->dataSize)
        throw std::runtime_error(
            "unexpected texdat size for " + texture.name);

    const std::uint64_t tex0Offset64 =
        0x20ull +
        static_cast<std::uint64_t>(info.texelBytes) +
        0x40ull;

    if (tex0Offset64 + 8ull > dataNode->dataSize)
        throw std::runtime_error(
            "TEX0 outside texdat record: " + texture.name);

    const std::size_t tex0Offset =
        static_cast<std::size_t>(tex0Offset64);
    ...
    if (info.tex0.psm != 0x13)
        throw std::runtime_error(
            "unsupported texture PSM (expected PSMT8) in " +
            texture.name);

    if (info.tex0.csm != 0)
        throw std::runtime_error(
            "CSM2 palette not yet supported in " +
            texture.name);
    ...
    const std::string& requested)
{
    ...
    if (!root)
        throw std::runtime_error(
            "texture archive has no 'textures' node");

    for (const auto& texture : root->children)
    {
        if (texture.name == requested)
            return ReadTextureNode(txr, texture);
    }
...
ReadTextures(const ZarArchive& txr)
{
    ...
    for (const auto& texture : root->children)
    {
        try
        {
            result.push_back(ReadTextureNode(txr, texture));
        }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (action == "--extract")
        {
            ...
            // Runtime 11.6: targeted extraction parses only the requested
            // texture node. Unsupported sibling textures can no longer
            // abort or crash a targeted UI extraction.
            const TextureInfo texture =
                ReadTextureByName(txr, requested);
