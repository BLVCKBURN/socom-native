    ...
    const std::vector<Rgba>& pixels)
{
    ...
    {
        throw std::runtime_error("runtime texture pixel count mismatch");
    }

    fs::path rawPath = pngPath;
    rawPath.replace_extension(".srtx");

    std::ofstream out(rawPath, std::ios::binary);
    if (!out)
        throw std::runtime_error(
            "cannot create " + rawPath.string());
    ...
    for (const auto& px : pixels)
    {
        ...
        out.write(
            reinterpret_cast<const char*>(rgba),
            sizeof(rgba));
    }

    if (!out)
        throw std::runtime_error(
            "failed writing " + rawPath.string());
}
    ...
    const std::vector<Rgba>& pixels)
{
    ...
    if (!out)
        throw std::runtime_error(
            "failed writing " + path.string());

    WriteRuntimeTextureSidecar(
        path,
        width,
        height,
        pixels);
