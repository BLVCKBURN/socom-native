...
ReadTextures(const ZarArchive& txr)
{
    ...
    auto skip = [&](const Node& texture, const std::string& reason)
    {
        ++skipped;
        // Some GameZ character banks mix multiple texture storage layouts.
        // Keep unsupported entries non-fatal so compatible PSMT8 textures can
        // still be extracted from the same archive.
        std::cout << "skip unsupported texture " << texture.name
                  << ": " << reason << '\n';
    };
    ...
    for (const auto& texture : root->children)
    {
        ...
        if (!dataNode)
        {
            skip(texture, "no texdat node");
            continue;
        }

        if (dataNode->dataSize < 0xA0)
        {
            skip(texture, "texdat record too small");
            continue;
        }
        ...
        {
            skip(texture, "not the linear 8-bit indexed layout");
            continue;
        }
        ...
        {
            skip(texture, "alternate texdat size/layout");
            continue;
        }
        ...
        {
            skip(texture, "unsupported texture PSM");
            continue;
        }

        if (info.tex0.csm != 0)
        {
            skip(texture, "CSM2 palette layout");
            continue;
        }

        if (info.tex0.cpsm != 0 && info.tex0.cpsm != 2)
        {
            skip(texture, "unsupported palette CPSM");
            continue;
        }
    ...
    if (skipped != 0)
    {
        std::cout << "Skipped " << skipped
                  << " unsupported texture record(s); continuing with "
                  << result.size() << " compatible texture(s).\n";
    }
...
static void WriteRuntimeTextureSidecar(
    const fs::path& outputPng,
    std::uint32_t width,
    std::uint32_t height,
    const std::vector<Rgba>& pixels)
{
    fs::path sidecar = outputPng;
    sidecar.replace_extension(".srtx");

    bool hasTransparency = false;
    for (const auto& px : pixels) {
        if (px.a != 255) { hasTransparency = true; break; }
    }

    std::ofstream out(sidecar, std::ios::binary);
    if (!out)
        throw std::runtime_error("cannot create runtime texture sidecar: " + sidecar.string());

    const char magic[8] = {'S','R','T','X','1','\0','\0','\0'};
    out.write(magic, sizeof(magic));
    const std::uint32_t flags = hasTransparency ? 1u : 0u;
    ...
    if (!out)
        throw std::runtime_error("runtime texture sidecar write failed: " + sidecar.string());
}
    ...
    const fs::path& outputDirectory)
{
    ...
    WriteRuntimeTextureSidecar(output, t.width, t.height, pixels);
